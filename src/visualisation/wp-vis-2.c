...
#define APPLICATION_ID  "org.mawsonlakes.wp.wp-vis-2"
...
// the neairest neighbours to calculaete the next state of the
//
//    1
//    |
//  2-+-0
...
static void
draw_statistics (GtkDrawingArea *area, cairo_t *cr, int width, int height, gpointer user_data) {
    char text[256]; // Magic number!!
    ...
    cairo_move_to(cr, xoffset_text, yoffset_text + i*20.0);
...
app_activate (GApplication *app, gpointer user_data) {
    ...
    build = gtk_builder_new_from_resource("/org/mawsonlakes/wp/wp-vis/wp-vis-2.ui");
    ...
    // Main: add drawable Area
     ...
     // Monitoring window
     GtkWidget *monitor;
     GtkWidget *statistics = gtk_drawing_area_new ();
     monitor = GTK_WIDGET(gtk_builder_get_object(build, "monitor"));

     gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(statistics), draw_statistics, NULL, NULL);
     gtk_window_set_child(GTK_WINDOW(monitor), statistics);
